# Changelog

## 1.0.3 - 2026-08-03

### Fixed

- Old Wizard now talks to the players on dedicated servers

## 1.0.2 - 2026-07-30

### Fixed

- Colossus Oak crash when cast on trees

## 1.0.1 - 2026-07-27

### Fixed

- Empower cantrip assets now showing up properly
- Empower cantrip buffs stacking fixed
- Old Wizards failing to spawn sometimes
- Spectral Slime size and behavior were tweaked
- Colossus Oak cantrip generating weird looking trees fixed
- Doubled the chances to get Ancient Scrolls

## 1.0.0 - 2026-07-19

### Added

- Empower cantrip was added
- Spectral Slime cantrip was added
- Ancient Scroll route for learning cantrips was added
- Wizard route for learning cantrips was added
- Bubble Up cantrip was added
- Colossus Oak cantrip was added

### Changed

- Spectral Bed, Spectral Boat and Lightning cantrip icons were redesigned
- Empower cantrips were rebalanced

### Fixed

- Spectral Donkey speed on water was fixed
- Spectral Donkey transparency fix
- Cantrip delay now works like in the base mod
- Spectral Armor protection scales with player tier now
- Spectral Armor not being transparent

## 0.4.0 - 2026-07-05

### Added
- Spectral Armor cantrip
- Villager Mind-Wiping cantrip

### Changed
- Force Consume cantrips was renamed to Devour

### Fixed
- Devour cantrip lightning effect fixed

### Changed

Memory Emeralds can now only be applied using the Villager Mind-Wiper cantrip

## 0.3.0 - 2026-06-28

### Added

Force Consume Cantrip icon [US-001](docs/user-stories.md)

Force Consume cantrip [US-001](docs/user-stories.md)

Spectral Bed cantrip [US-003](docs/user-stories.md)

Spectral Donkey cantrip [US-005](docs/user-stories.md)

Spectral Boat cantrip [US-006](docs/user-stories.md)

## 0.2.0 - 2026-06-21

### Added
Spectral Bed cantrip [US-003](docs/user-stories.md)

Lightning cantrip [US-004](docs/user-stories.md)

Spectral Donkey cantrip [US-005](docs/user-stories.md)

Spectral Boat cantrip [US-006](docs/user-stories.md)

Spectral Chest now opens when Shift+Right clicking the Spectral Donkey and Spectral Boat

Re-designed icons for Force Consume cantrip

### Changed
Proper Lightning spell (US-004) icon was added

Re-designed three existing cantrip icons

### Deprecated

### Removed

### Fixed

Spectral Donkey cantrip [US-005](docs/user-stories.md) (shared inventory opening)

Spectral Boat cantrip [US-006](docs/user-stories.md) (shared inventory opening)

Spectral Bed cantrip [US-003](docs/user-stories.md) now summons correctly when in a dimension where beds aren't allowed

Spectral Donkey cantrip [US-005](docs/user-stories.md) (no longer drops saddle on death)

### Security
